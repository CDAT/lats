...
int gaddes (char *name, struct gafile *pfi, int mflag) {
  ...
  char rec[256], mrec[256], *ch, *dn, *pos, *sname ; /*mf mf*/
  ...
  if(sname !=NULL) {
    getwrd (pfi->dnam,sname,199);
    free(sname);
  } else {
    getwrd (pfi->dnam,name,199);
  }
...
/*mf---------mf*/
  ...
  while (fgets(rec,256,descr)!=NULL) {
    strcpy (mrec,rec);
    lowcas(rec);
...
/*mf 961204 set the global calendar and check if we are trying to change with a new file... mf*/
...
void free_io_std(IO_STD**) ;
  ...
  if (pfi->ppi) free(pfi->ppi);
  if (pfi->ppf[0]) free(pfi->ppf[0]);
  if (pfi->ppf[1]) free(pfi->ppf[1]);
  if (!flag) {
...
/* Routine to calculate the interpolation constants needed for
   the implicit interpolation from pre-projected grids to lat-lon. */

int gappcn (struct gafile *pfi) {
int size,i,j,ii,jj;
...
int *ioff;
  ...
  pfi->ppi = (int *)malloc(sizeof(int)*size);
  if (pfi->ppi==NULL) goto merr;
  ...
  if ( pfi->ppflag == 4) {
    pfi->ppf[2] = (float *)malloc(sizeof(float)*size);
    if (pfi->ppf[2]==NULL) goto merr;
    etarot=pfi->ppf[2];
  }
